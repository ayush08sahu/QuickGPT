...
const Community = () => {
  ...
  return (
    <div className='p-6 pt-12 2xl:px-20 w-full mx-auto h-full overflow-y-scroll'>
      <h2 className='text-xl fond-semibold mb-6 text-gray-800 dark:text-purple-100'>Community Images</h2>
      {images.length > 0 ? (
        <div className='flex flex-wrap mx-sm:justify-center gap-5'>
          {images.map((item, index) => (
            <a key={index} href={item.imageUrl} target='_blank' className='relative group block rounded-lg overflow-hidden border bordoer-gray-200 dark:border-purple-700 shadow-sm hover:shadow-md transition-shadow duration-300' >
